...
class Scene {
  ...
  async listAll() {
    const xmlString = await this.internal.exec('AppGetPropertyAsync', 'sceneconfig');
    const result = parser.parse(xmlString, {
      attributeNamePrefix: '',
      ignoreAttributes: false,
    });

    const { configuration: { placement } } = result;
    ...
    if (Array.isArray(placement)) {
      ...
      return [{ id: placement.id, index: 0 }];
    }
  ...
  async getItems(id: string): Promise<Item[]> {
    const xmlString = await this.internal.exec('AppGetPropertyAsync', `sceneconfig:${id}`);
    ...
    const items =
      Array.isArray(sceneObject.placement.item)
